...
var playAudio = function (data) {
  try {
    ...
    var audioBuffer = audioContext.createBuffer(
      captureCfg.channels,
      data.length / captureCfg.channels,
  ...
  try {
    if (window.audioinput && !audioinput.isCapturing()) {
      var audioSourceElement = document.getElementById('audioSource'),
        audioSourceType =
          audioSourceElement.options[audioSourceElement.selectedIndex].value;
      ...
      captureCfg = {
        ...
        channels: parseInt(
          document.querySelector('input[name="channels"]:checked').value,
        ),
